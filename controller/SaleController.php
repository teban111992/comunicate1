  ...
  function showForm($privileges, $tableName, $userLevel) {
    ...
    if ($data->getLink() != NULL) {
     ...
     if($data->getResultset() !=NULL){
         ...
         $header = array("idbill", "codeproduct", "quantity");
         ...
         $formatter->tableHeaders($header);
          ...
          while($row = mysqli_fetch_array($data->getResultset(),MYSQLI_ASSOC)){
                ...
                echo "\t\t<tr>\n";
                $formatter->addTableField($sale->getIdBill());
                $formatter->addTableField($sale->getCodeProduct());
                $formatter->addTableField($sale->getQuatity());
